...
if (isset($_POST["import"])) {
  ...
  if (in_array($_FILES["file"]["type"], $allowedFileType)) {
    ...
    for ($i = 1; $i < $sheetCount; $i ++) {
      ...
      if ( !empty($dato_01) || !empty($dato_02) || !empty($dato_03) || !empty($dato_04) ) {
        $query = "insert into finanzas_stage_ppto(dea, anio, mes, monto) values(?, ?, ?, ?)";
        $paramType = "ssss";
        $paramArray = array($dato_01, $dato_02, $dato_03, $dato_04);
